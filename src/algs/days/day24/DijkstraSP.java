...
public class DijkstraSP {
	...
	public DijkstraSP(AbstractWeightedDigraph G, int s) {
	...
	public static void main(String[] args) {
		...
		// inclass example
		AbstractWeightedDigraph sample = new DigraphAdjacencyList(new In(new File ("day24.txt")));
		DijkstraSP sample_sp = new DijkstraSP(sample, 0);
		for (int v = 0; v < sample.V(); v++) {
			StdOut.print(v + ":" + sample_sp.distTo(v) + "\t");
		...
		for (int n = 2; n <= 64; n *= 2) {
			...
			AbstractWeightedDigraph G = new DigraphAdjacencyList(in);
			int s = 0;
