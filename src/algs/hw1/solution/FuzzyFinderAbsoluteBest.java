 ...
 * 
 * BENDS THE RULES BY RECORDING TOTAL NUMBER FOUND (AV-1), and returning NULL once
 * N*N have been found.
 */
public class FuzzyFinderAbsoluteBest implements IFuzzySquareFinder {
	int numFound = 0;
	...
	public Coordinate find(FuzzySquare fs, int target) {
		...
		// HACK! Takes advantage of ordering....
		if (numFound == fs.N*fs.N) { return null; }
	...
	public static void main(String[] args) {
		...
		for (int s = 0; s < 1000; s++) {
			...
			fs2.solver(new FuzzyFinderAbsoluteBest());
		...
		for (int i = 3; i < 20; i++) {
			...
			fs.solver(new FuzzyFinderAbsoluteBest());
