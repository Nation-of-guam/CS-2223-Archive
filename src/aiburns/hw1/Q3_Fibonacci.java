...
public class Q3_Fibonacci {
	...
	static long fibonacci_improved(long n) {
		...
		return (2*fibImp(n-2))+fibImp(n-3);
	}

	static long fibImp(long n){
		return fibonacci_improved(n);
	}
	...
	static long lucas_improved(long n) {
		...
		return (2*fibonacci_improved(n-1))+fibImp(n);
	}

	static long lucImp(long n){
		return lucas_improved(n);
	}

	public static long floorOver2(long a){
	...
	public static void improved_implementation(int max) {
		...
		for (int n = 0; n < max; n++) {
			double timef = 10*time_fibonacci_recursive(n);
			double timefi = 10*time_fibonacci_improved(n);
